...
type ApiRequestCallback<T> = (response: ApiResponse<T>) => void;
type ApiRequestFunc<T> = (callback: ApiRequestCallback<T>) => void;

function promisify<T>(func: ApiRequestFunc<T>): () => Promise<T> {
  return () =>
